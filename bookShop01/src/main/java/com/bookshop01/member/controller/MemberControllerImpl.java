...
public class MemberControllerImpl  extends BaseController implements MemberController{
	...
	//장바구니를 동기화한다.
	private void syncCartList(HttpServletRequest request, HttpServletResponse response)
	                           throws Exception{
		...
		//상품 번호를 분리한 후 상품번호로 각 상품 정보를 조회한다.
		if(cart_value==null || cart_value.length()==0){
			return;
		}else{
			String[] goods_ids=cart_value.split("-");
			ArrayList<CartBean> cart_list_cookie=new ArrayList<CartBean>();
